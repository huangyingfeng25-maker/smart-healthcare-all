...
public class HospitalReceive {
    ...
    public void receiver(OrderMqVo orderMqVo, Message message, Channel channel) throws IOException {
        ...
        Schedule schedule = scheduleService.getScheduleId(scheduleId);//含医院名称和课室名称
        ...
        scheduleService.update(schedule);//更新mongo数据库的排班集合数据

        MsmVo msmVo=orderMqVo.getMsmVo();
        if (msmVo!=null){
            //发mq消息，通知消费者端给用户发手机短信
            rabbitService.sendMwssage(MqConst.EXCHANGE_DIRECT_MSM,MqConst.ROUTING_MSM_ITEM,msmVo);
        }
    }
}
